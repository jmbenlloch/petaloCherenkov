"""
Module NumpyAlgebra defines vector and matrices from numpy 
"""
import numpy as np
import sys
eps=1e-10
...
class NPMatrix(object):
    ...
    def __init__(self,obj):
        ...
        elif isinstance(obj,NPMatrix) ==True or isinstance(obj,NPVector) ==True:
            i,j = np.shape(obj.M)
            m=np.zeros(i*j)
    ...
    def __add__(self, other): 
        ...
        if isinstance(other,NPMatrix) == True or isinstance(other,NPVector) == True:
            m = NPMatrix(self)
            m.M += other.M
            return m
            ...
            print "cannot add a NPMatrix to type ",type(other)
            sys.exit(-1) 
    ...
    def __mul__(self, other): 
        ...
        if isinstance(other,NPMatrix) == True:
            m = NPMatrix(self)
            m.M = np.dot(self.M,other.M)
            return m
        elif isinstance(other,NPVector) == True:
            m = NPVector(other)
            m.M = np.dot(self.M,other.M)
            return m
        elif isinstance(other,int) ==True or isinstance(other,float) == True: 
            ...
            print "cannot multiply a NPMatrix to type ",type(other)
            sys.exit(-1) 
...
class NPVector(object):
    ...
    def __init__(self,obj):
        """
        Takes as an argument a list or another NPVector.
        ...
        if isinstance(obj,list) ==True:
            ...
            if not col == 1:
                print "A NPVector takes as input a list of number"
                print "for example: v = NPVector([1,2,3])"
                sys.exit(-1)

        elif isinstance(obj,NPVector) ==True or isinstance(obj,NPMatrix) ==True:
            i,j = np.shape(obj.M)
            if j != 1:
                print "A NPVector must be a column vector"
            ...
            print """input  must be a list v = NPVector([1,2,3])
                     or another NPVector
                  """
            sys.exit(-1)
    ...
    def __add__(self, other): 
        ...
        if isinstance(other,NPVector) ==True or isinstance(other,NPMatrix) ==True:
            ...
            print "cannot add the NPVector to type ",type(other)
            sys.exit(-1)
    ...
    def __mul__(self, other): 
        ...
        """
        product of two NPVectors ---> scalar product (dot)
        Product by a matrix H*a ---> a NPVector
        Product by a matrix a*H ---> a NPMatrix
        """
        ...
        if isinstance(other,NPMatrix) == True:
            ...
            print "cannot multiply a NPVector to type ",type(other)
            sys.exit(-1) 
...
def testVector():
    v1 = NPVector([1.,2.,3.])  #notice: 1.,2.,3.] to get floats
    vv2 = NPVector([2.,3.,4.]) 
    v1pv2=NPVector([3.,5.,7.])
    v1mv2=NPVector([-1.,-1.,-1.])
    v2mv1=NPVector([1.,1.,1.])
...
def testMatrix():
    ...
    m =NPMatrix([[1.,2.,3.],[0.,1.,4.],[5.,6.,0.]])
    mp10 =NPMatrix([[10.,20.,30.],[0.,10.,40.],[50.,60.,0.]])
    ma10 =NPMatrix([[11.,12.,13.],[10.,11.,14.],[15.,16.,10.]])
    mm =NPMatrix([[16.,22.,11.],[20.,25.,4.],[5.,16.,39.]])
    mT =NPMatrix([[1.,0.,5.],[2.,1.,6.],[3.,4.,0.]])
    mI=NPMatrix([[-24.,  18.,   5.],[20., -15.,  -4.],[-5.,   4.,   1.]])
    mpmT=NPMatrix([[ 2.,  2.,  8.],
        [ 2.,  2., 10.],
        [ 8., 10.,  0.]])

    I =NPMatrix([[1.,0.,0.],[0.,1.,0.],[0.,0.,1.]])
...
def testKHVector():
    ...
    sigma_x=0.1
    sigma_y=0.2
    cov=NPMatrix([[sigma_x**2,0],
                   [0,sigma_y**2]])

    print "hit is a NPVector",isinstance(hit,NPVector)
    print "hit =",hit

    print "cov is a NPMatrix",isinstance(cov,NPMatrix)
    print "cov =",cov
...
def testMatrixMult():
    ...
    m1 = NPMatrix([[1.,2.,-3.,4.],[0.,1.,-4.,3.],[5.,-6.,0.,2.],[-2.,4.,1.,-5.]])
    m2 = NPMatrix([[2.,-5.,1.,-2.],[2.,1.,1.,2.],[4.,-3.,3.,2.],[1.,1.,6.,-3.]])
    v1 = NPVector([2.,3.,-6.,1.])
    v2 = NPVector([0.,2.,2.,4.])

    prod1_expect = NPMatrix([[-2.,10.,18.,-16.],[-11.,16.,7.,-15.],[0.,-29.,11.,-28.],[3.,6.,-25.,29.]]) # m1*m2
    prod2_expect = -2  # v1*v2
    prod3_expect = NPVector([30.,30.,-6.,-3.])
    prod4_expect = NPMatrix([-13.,12.,-7.,-13.])
