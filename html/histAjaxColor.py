...
p = figure(x_range=(100,1300),y_range=(100,1300))
#colorPlot = p.annulus(x=[0], y=[0], color=[0],inner_radius=0, outer_radius=[0],fill_alpha=0.5,line_color=None)
colorPlot = p.annulus(x=[0], y=[0],color=[0],inner_radius=0, outer_radius=[0],fill_alpha=0.5,line_color=None)

callbackHist = CustomJS(args=dict(
            source=hist.data_source,
            cPSrc=colorPlot.data_source,
            txtSrc=txt.data_source,
            txtMinSrc=txtMinWL.data_source,
            txtMaxSrc=txtMaxWL.data_source,
            sPDE=sliderPDE,
            sMinWL=sliderMinWL,
            sASIC=sliderASIC,
            sSPTR=sliderSPTR), code="""
(function(){
...
url = 'http://localhost:5000/dataColor/210/' + pde + '/' + minwl + '/1500/' + asic + '/' + sptr
$.getJSON(url, function(data) {
        ...
        cPData = cPSrc.get('data');
        var x = cPData['x']
        var y = cPData['y']
        var colors = cPData['fill_color']
        var radius = cPData['outer_radius']
        x.splice(0,x.length)
        y.splice(0,y.length)
        radius.splice(0,radius.length)
        for (i = 0; i < data['x'].length; i++) {
            x[i] = data.x[i]
            y[i] = data.y[i]
            radius[i] = data.radii[i]
...
output_file("color.html", title="color_scatter.py example")
